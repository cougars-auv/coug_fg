...
TEST(WrenchDynamicsFactorArmTest, Residual) {
  ...
  gtsam::Vector3 const target_force = target_T_sensor.rotation().matrix() * control_force;
  gtsam::Vector3 const vel_target_i = pose_i.rotation().matrix().transpose() * vel_i;
  gtsam::Vector3 const drag_force = -(
      linear_drag * vel_target_i + quad_drag * vel_target_i.cwiseProduct(vel_target_i.cwiseAbs()));
  gtsam::Vector3 const vel_target_pred =
      vel_target_i + mass.inverse() * (target_force + drag_force) * dt;

  // Pick vel_j so the residual is the offset
  gtsam::Vector3 const offset(0.02, -0.01, 0.03);
  gtsam::Vector3 const vel_j = pose_j.rotation().matrix() * (vel_target_pred + offset);
