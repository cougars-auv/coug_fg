...
enum class SolverType : std::uint8_t { kIncrementalFixedLagSmoother, kIsam2, kLevenbergMarquardt };
...
inline auto parseSolverType(const std::string& solver_type) -> SolverType {
  ...
  if (solver_type == "ISAM2") {
    return SolverType::kIsam2;
  }
  if (solver_type == "LevenbergMarquardt") {
    return SolverType::kLevenbergMarquardt;
  }
  throw std::invalid_argument("Unknown solver_type: " + solver_type);
}

inline auto parseRobustKernel(const std::string& robust_kernel) -> RobustKernel {
...
inline auto parseKeyframeSource(const std::string& keyframe_source) -> KeyframeSource {
  ...
  if (keyframe_source == "Depth") {
    return KeyframeSource::kDepth;
  }
  if (keyframe_source == "Timer") {
    return KeyframeSource::kTimer;
  }
